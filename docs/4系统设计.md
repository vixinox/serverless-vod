...
系统共包含 19 张数据表, 以下逐表列出各字段的名称、类型及约束信息。
...
| ipAddress | String | |
| userAgent | String | |
| userId | String | FK → user.id（CASCADE）, NOT NULL |
...
**表 4-7 user_settings 表**

| 字段名 | 类型 | 约束 |
| --- | --- | --- |
| userId | String | PK, FK → user.id（CASCADE） |
...
| ownerId | String | UNIQUE, FK → user.id（CASCADE） |
...
**表 4-9 Subscription 表**

| 字段名 | 类型 | 约束 |
| --- | --- | --- |
| id | String | PK, DEFAULT cuid() |
| subscriberId | String | FK → user.id（CASCADE）, INDEX |
| channelId | String | FK → Channel.id（CASCADE）, INDEX |
| createdAt | DateTime | NOT NULL, DEFAULT now() |
...
**表 4-11 VideoReaction 表**

| 字段名 | 类型 | 约束 |
| --- | --- | --- |
| userId | String | PK（复合）, FK → user.id（CASCADE） |
| videoId | String | PK（复合）, FK → Video.id（CASCADE）, INDEX |
...
| userId | String | FK → user.id（CASCADE）, INDEX |
| videoId | String | FK → Video.id（CASCADE）, INDEX |
| parentId | String | FK → Comment.id（CASCADE, 自引用）, INDEX |
...
**表 4-13 CommentReaction 表**

| 字段名 | 类型 | 约束 |
| --- | --- | --- |
| userId | String | PK（复合）, FK → user.id（CASCADE） |
| commentId | String | PK（复合）, FK → Comment.id（CASCADE）, INDEX |
| reactionType | ReactionType | NOT NULL |
| createdAt | DateTime | NOT NULL, DEFAULT now() |
...
| --- | --- | --- |
| id | String | PK, DEFAULT cuid() |
| ownerId | String | FK → user.id（CASCADE）, INDEX |
| title | String | NOT NULL |
| description | String | |
...
**表 4-15 PlaylistItem 表**

| 字段名 | 类型 | 约束 |
| --- | --- | --- |
| id | String | PK, DEFAULT cuid() |
| playlistId | String | FK → Playlist.id（CASCADE）, INDEX |
| videoId | String | FK → Video.id（CASCADE）, INDEX |
| addedById | String | FK → user.id（CASCADE）, INDEX |
| position | Int | NOT NULL |
| createdAt | DateTime | NOT NULL, DEFAULT now() |
| — | — | UNIQUE (playlistId, position)；UNIQUE (playlistId, videoId)；INDEX (playlistId, createdAt) |

**表 4-16 UploadSession 表**

| 字段名 | 类型 | 约束 |
| --- | --- | --- |
| id | String | PK, DEFAULT cuid() |
| videoId | String | FK → Video.id（CASCADE）, INDEX |
| uploaderId | String | FK → user.id（CASCADE）, INDEX |
| storageBucket | String | NOT NULL |
...
| inputBucket | String | NOT NULL |
| inputKey | String | NOT NULL |
| outputBucket | String | NOT NULL |
| outputPrefix | String | NOT NULL |
| status | TranscodeJobStatus | NOT NULL, DEFAULT QUEUED |
| attempt | Int | NOT NULL, DEFAULT 0 |
| maxAttempts | Int | NOT NULL, DEFAULT 3 |
| pipelineStage | String | |
| lastError | String | |
| queuedAt | DateTime | NOT NULL, DEFAULT now() |
| startedAt | DateTime | |
| finishedAt | DateTime | |
| createdAt | DateTime | NOT NULL, DEFAULT now() |
| updatedAt | DateTime | NOT NULL |
...
**表 4-19 VideoPlaybackEvent 表**

| 字段名 | 类型 | 约束 |
| --- | --- | --- |
| id | String | PK, DEFAULT cuid() |
| videoId | String | FK → Video.id（CASCADE）, NOT NULL |
| userId | String | FK → user.id（SetNull） |
...
| likesGained | Int | NOT NULL, DEFAULT 0 |
| dislikesGained | Int | NOT NULL, DEFAULT 0 |
| commentsGained | Int | NOT NULL, DEFAULT 0 |
| — | — | UNIQUE (videoId, date)；INDEX date |

**表 4-21 ChannelDailyStat 表**

| 字段名 | 类型 | 约束 |
| --- | --- | --- |
| id | String | PK, DEFAULT cuid() |
| channelId | String | FK → Channel.id（CASCADE）, NOT NULL |
| date | Date | NOT NULL |
| views | BigInt | NOT NULL, DEFAULT 0 |
| watchTimeSeconds | BigInt | NOT NULL, DEFAULT 0 |
| subscribersGained | Int | NOT NULL, DEFAULT 0 |
| subscribersLost | Int | NOT NULL, DEFAULT 0 |
| videosPublished | Int | NOT NULL, DEFAULT 0 |
| — | — | UNIQUE (channelId, date)；INDEX date |

此外, 数据库通过 triggers.core.sql 中的触发器自动维护评论数、点赞数和订阅数等累计计数字段, 通过 rollup.daily.sql 中的 sp_rollup_daily_stats 存储过程按日期范围汇总播放事件、互动和发布数据, 生成 VideoDailyStat 与 ChannelDailyStat 两张日级统计表。
