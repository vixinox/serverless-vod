...
### 3.1 Job Timeline 查询（Read）

1. 目标。
说明：给工作室页面提供单视频任务时间线，展示 `queuedAt/startedAt/finishedAt`、`pipelineStage` 变更轨迹、错误摘要。

2. 建议接口。
说明：`GET /api/studio/videos/[videoId]/jobs`，返回最近 N 条任务及阶段节点。

3. UI 收益。
说明：用户能看到“卡在下载/转码/上传哪一步”，减少“转圈但不知发生什么”的感知。
